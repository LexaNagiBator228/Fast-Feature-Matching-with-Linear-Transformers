...
parser.add_argument('--image_path', type=str,
                        help='The image path of the testing image', default='./media/ref_car.jpg')
parser.add_argument('--video_path', type=str,
                        help='The video path of the testing image', default='./media/vid_car.MOV' ) 
...
while cap.isOpened():
    ...
    with torch.no_grad():
        ...
        im1 = cv2.resize(cv2.imread(im_path, 0), (512,512))


        gray = cv2.resize(cv.cvtColor(frame, cv.COLOR_BGR2GRAY), (512, 512))
        im2 = gray
        m1 = im1
        m2 = im2
    ...
    if(args.vis):
        ...
        cv2.waitKey(0)
if (args.save_video):
    out = cv2.VideoWriter('./results/res.avi',cv2.VideoWriter_fourcc(*'DIVX'), 10, (512*2, 512))
